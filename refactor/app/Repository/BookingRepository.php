...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!$user) {
            return ['emergency_jobs' => [], 'normal_jobs' => [], 'user' => null, 'usertype' => ''];
        }
        ...
        if ($user->is('customer')) {
            $jobs = $user->jobs()
                ->with('user.user_meta', 'user.average', 'translator_job_rel.user.average', 'language', 'feedback')
                ->whereIn('status', [JobStatusEnum::STARTED, JobStatusEnum::ASSIGNED, JobStatusEnum::PENDING])
                ->orderBy('due', 'asc')
                ->get();
            $usertype = 'customer';
        ...
        return ['emergency_jobs' => $emergency_jobs, 'normal_jobs' => $normal_jobs, 'user' => $user, 'usertype' => $usertype];
    }
...
//            $jobs['total'] = $totaljobs;
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $user, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
        }
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($data['immediate'] != 'no' && isset($data['duration']) && empty($data['duration'])) {
            $response['status'] = 'fail';
            $response['message'] = "Du måste fylla in alla fält";
        ...
        if (isset($data['due_date']) && empty($data['due_date']) && $data['immediate'] == 'no') {
            $response['status'] = 'fail';
            $response['message'] = "Du måste fylla in alla fält";
            $response['field_name'] = "due_date";
            return $response;
        }
        if (isset($data['due_time']) && !($data['due_time']) && $data['immediate'] == 'no') {
            $response['status'] = 'fail';
            $response['message'] = "Du måste fylla in alla fält";
            $response['field_name'] = "due_time";
            return $response;
        }
        if (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type']) && $data['immediate'] == 'no') {
            $response['status'] = 'fail';
            $response['message'] = "Du måste göra ett val här";
            $response['field_name'] = "customer_phone_type";
            return $response;
        }
        if (isset($data['duration']) && empty($data['duration']) && $data['immediate'] == 'no') {
            $response['status'] = 'fail';
            $response['message'] = "Du måste fylla in alla fält";
            $response['field_name'] = "duration";
            return $response;
    ...
    {
        if (empty($post_data) || !isset($post_data["job_id"])) {
            return null;
        }
        ...
        $diff = $complete_date->diff($start);
        $job->end_at =  Carbon::now();
        $job->status = JobStatusEnum::COMPLETED;
        $job->session_time = $diff->format('%h:%i:%s');
        $job->save();
        $this->sendJobEndMail($job, $post_data);
    }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $languages = UserLanguages::where('user_id', '=', $user_id)->get()->pluck('lang_id')->toArray();
        $job_ids = Job::getJobs($user_id, $job_type, 'pending', $languages, $user_meta->gender, $user_meta->translator_level)
            ->pluck('id')
            ->toArray();

        return Job::whereIn('id', $job_ids)->where(function ($query) use ($user_id) {
            $query->where('customer_phone_type', 'yes')
                ->orWhere('customer_physical_type', 'no')
                ->orWhereNull('customer_phone_type');
        })->whereHas('town', function ($query) use ($user_id) {
    ...
    {
        $users = User:: User::where('user_type', '2')
            ->where('status', '1')->where('id', '!=', $exclude_user_id);
        ...
        $msg_contents .= $data['immediate'] == 'no' ? $data['due'] : '';
        $msg_text = [
            "en" => $msg_contents
        ];
...
//        }
    ...
    private function changeStartedStatus($job, $data)
    {
        ...
        $this->sendSessionEndedEmail($user, $job, $email, $subject, $session_time, 'faktura');

        $user = $job->translatorJobRel->where('completed_at', null)->where('cancel_at', null)->first();
        $email = $user->user->email;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $this->sendSessionEndedEmail($user, $job, $email, $subject, $session_time, 'lön');
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (is_null($current_translator) || (!isset($data['translator'])
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($user && $user->user_type == config('app.SUPERADMIN_ROLE_ID')) {
            $allJobs = $this->userList($request, $limit);
        } else {
        ...
        if ($request->has('feedback') && $request->boolean('feedback')) {
            $allJobs->where('ignore_feedback', '0');
            $allJobs->whereHas('feedback', function ($q) {
                $q->where('rating', '<=', '3');
            });
            if ($request->has('count') && $request->boolean('count')) {
        ...
        if ($request->has('customer_email') && is_array($request->customer_email)) {
            // assuming job have relation with users.
            $allJobs->whereHas('users', function ($q) use ($request) {
                $q->whereIn('email', $request->customer_email);
            });
        }

        if ($request->filled('filter_timetype') && $request->filter_timetype === 'created') {
        ...
        if ($request->filled('filter_timetype') && $request->filter_timetype === 'due') {
            if ($request->filled('from')) {
                $from = Carbon::parse($request->from);
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    private function userList($request, $limit, $allJobs)
    {
        ...
        if ($request->filled('will_expire_at')) {
            $allJobs->where('will_expire_at', '>=', $request->expired_at);
        }

        if ($request->has('translator_email') && is_array($request->translator_email)) {
            // assuming TranslatorJobRel have relation with users.
            $translator_job_rel_ids = TranslatorJobRel::whereHas('users', function ($q) use ($request) {
                $q->whereNull('cancel_at')->whereIn('email', $request->translator_email);
            })->get()->pluck('id')->toArray();
            $allJobs->whereIn('id', $translator_job_rel_ids);
        }

        if ($request->has('physical') && is_array($request->physical)) {
            $allJobs->whereIn('physical', $request->physical)->where('ignore_physical', 0);
        }

        if ($request->filled('phone')) {
            $allJobs->where('customer_phone_type', $request->phone);
            if ($request->filled('physical')) {
                $allJobs->where('ignore_physical_phone', 0);
            }
        }

        if ($request->filled('flagged')) {
            $allJobs->where('flagged', $request->flagged)->where('ignore_flagged', 0);
        }

        if ($request->filled('distance') && $request->distance === 'empty') {
            $allJobs->whereDoesntHave('distance');
        }

        if ($request->filled('yes') && $request->salary === 'yes') {
            $allJobs->whereDoesntHave('user.salaries');
        }

        if ($request->filled('count') && $request->count === 'true') {
            return ['count' => $allJobs->count()];
        }

        if ($request->filled('consumer_type')) {
            $allJobs->whereHas('user.user_meta', function ($q) use ($request) {
    ...
    private function sendJobEndMail($job, $post_data)
    {
        $user = $job->user()->get()->first();
        $email = !empty($job->user_email) ? $job->user_email : $user->email;
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        ...
        $tr->completed_at = $job->end_at;
        $tr->completed_by = $post_data['userid'];
        $tr->save();
    }

    private function sendSessionEndedEmail($user, $job, $email, $subject, $session_time, $for_text)
    {
        $name = $user->name;
        $data = [
            'user' => $user,
